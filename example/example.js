(function($) {
  ...
  var lookup = function(ctx, prefix) {
  ...
  wrds = $('#paragraph').html().split(/\s/);
  wrds.forEach(function(wrd) {
    if (!(wrd.match(/[\d\s]/))) {
      console.log(wrd);
      ex.addWord(wrd.toLowerCase());
    }
  });

  ctx = JSON.parse(ex.stringify());

  $('#autocomplete').submit(function(e) {
    e.preventDefault();
    var val = $('#theForm').val();
    var words = lookup(ctx,val);
    $('#words').html("");
    words.forEach(function(word) {
      $('#words').append('<li class="word">'+word+'</li>');
    });
    console.log(lookup(ctx,val));
  });

  $('#words').click(function(e) {
    var $target = $(e.target);
    $('#theForm').val($target.html());
    $('#words').html("");
  });
